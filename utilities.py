...
#-----------------------------------------------------------------------------------------------------------
def download_MCMIPF(satellite, yyyymmddhhmn, path_dest):
  ...
  # TRY WITH M6
  # File structure
  prefix = f'{product_name}/{year}/{day_of_year}/{hour}/OR_{product_name}-M6_{sat}_s{year}{day_of_year}{hour}{min}'
  ...
  if 'Contents' not in s3_result: 
    # File structure
    prefix = f'{product_name}/{year}/{day_of_year}/{hour}/OR_{product_name}-M4_{sat}_s{year}{day_of_year}{hour}{min}'
    # Seach for the file on the server
    s3_result = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=prefix, Delimiter = "/")
  ...
  if 'Contents' not in s3_result: 
    # File structure
    prefix = f'{product_name}/{year}/{day_of_year}/{hour}/OR_{product_name}-M3_{sat}_s{year}{day_of_year}{hour}{min}'
    # Seach for the file on the server
    s3_result = s3_client.list_objects_v2(Bucket=bucket_name, Prefix=prefix, Delimiter = "/")
  ...
  if 'Contents' not in s3_result: 
    ...
    for obj in s3_result['Contents']: 
      ...
      # Download the file
      if os.path.exists(f'{path_dest}{file_name}.nc'):
        print(f'File {path_dest}{file_name}.nc already exists')
      else:
        print(f'Downloading file {path_dest}{file_name}.nc')
        s3_client.download_file(bucket_name, key, f'{path_dest}/{file_name}.nc')
  return f'{file_name}'
